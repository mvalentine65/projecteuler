    ...
    def horizontal_search(self, length: int = 4) -> int:
        ...
        for row in self.grid:
            ...
            for x in range(self.width - length + 1):
                max_seen = max(max_seen, prod(row[x : x + length]))
        return max_seen

    def vertical_search(self, length: int = 4) -> int:
        max_seen = 0
        for x in range(self.width):
            column = [row[x] for row in self.grid]
            # ditto
            for y in range(self.height - length + 1):
                max_seen = max(max_seen, prod(column[y : y + length]))
        return max_seen

    def diagnol_search(self, length: int = 4) -> int:
        max_seen = 0
        for y in range(self.height - length + 1):
            for x in range(self.width - length + 1):
                current1 = prod([self.grid[y + i][x + i] for i in range(length)])
                current2 = prod(
                    [self.grid[y + i][x + length - 1 - i] for i in range(length)]
                )
                max_seen = max((current1, current2, max_seen))
        return max_seen

    def search(self, length: int = 4) -> int:
        return max(
            self.horizontal_search(length),
            self.vertical_search(length),
            self.diagnol_search(length),
        )
...
def eleven() -> int:
    ...
    # the grid is 20x20, I don't need to retain the original structure
    # flatten
